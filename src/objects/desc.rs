...
impl From<DescMode> for usize {
    fn from(d: DescMode) -> Self { d as usize }
}
...
pub fn obj_desc_name_format(fmt: &str, modstr: Option<&str>, pluralize: bool) -> String {
    ...
    for c in fmt.chars() {
        match mode {
            ...
            FormatMode::Custom (CustomMode::Singular (so_far, stored)) => {
                match c {
                    '|' => {
                        let new_stored = 
                            if pluralize == false {
                    ...
                    _ => {
                        mode = FormatMode::Custom (CustomMode::Singular (so_far + &(c.to_string()), stored));
                    }
                }
            },
            FormatMode::Custom (CustomMode::Plural (so_far, stored)) => {
                match c {
                    '|' => {
                        let new_stored = if pluralize == true { stored + &so_far } else { stored.clone() };
                        mode = FormatMode::None (new_stored);
                    },
                    _ => {
                        mode = FormatMode::Custom (CustomMode::Plural (so_far + &(c.to_string()), stored));
                    }
                }
            }
            FormatMode::Skip (stored) => {
                match c {
                    '&' | ' ' => {
                        mode = FormatMode::Skip (stored);
                    },
